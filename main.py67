...
user_translate_mode = {}  # Added for translate functionality

# Dictionary to store the last transcription result for each user
last_transcription = {}
...
def start_handler(message):
    ...
    user_translate_mode[user_id_str] = None
...
def help_handler(message):
    ...
    user_translate_mode[user_id] = None
...
def handle_voice_command(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    user_pitch_input_mode[user_id_for_settings] = None
    user_rate_input_mode[user_id_for_settings] = None
    user_translate_mode[user_id_for_settings] = None
    ...
    try:
        ...
        if res['status'] == 'completed':
            ...
            if not text:
                ...
                markup = InlineKeyboardMarkup()
                markup.add(
                    InlineKeyboardButton("Summarize", callback_data=f"summarize|{original_message_id}"),
                    InlineKeyboardButton("Translate", callback_data=f"translate|{original_message_id}")
                )
                if len(text) <= 4000:
                    target_bot.send_message(chat_id, text, reply_to_message_id=original_message_id, reply_markup=markup)
                    last_transcription[str(original_message_id)] = text
                else:
                    # Store the transcription text in a .txt file
                    temp_file_path = f"transcription_{uuid.uuid4()}.txt"
                    with open(temp_file_path, 'w', encoding='utf-8') as f:
                        f.write(text)
                    
                    # Send the .txt file to the user
                    with open(temp_file_path, "rb") as f:
                        target_bot.send_document(chat_id, f, caption="Your transcript is too long. Here is the file:", reply_to_message_id=original_message_id, reply_markup=markup)
                    
                    # Store the file path instead of the text
                    last_transcription[str(original_message_id)] = temp_file_path
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if user_translate_mode.get(uid) is not None:
        try:
            language_name = message.text.strip().capitalize()
            original_message_id = user_translate_mode.pop(uid)
            
            # Retrieve transcription content, handling both strings and file paths
            transcription_content = get_transcription_content(original_message_id)

            if transcription_content is None:
                bot.send_message(message.chat.id, "❌ Sorry, the transcription text could not be found.")
                return

            bot.send_message(
                chat_id=message.chat.id,
                text="Processing... This might take a moment.",
            )

            prompt = f"Translate the following text to {language_name}:\n\n{transcription_content}"
            gemini_response = call_gemini_api(prompt)
            
            send_gemini_response_to_user(message.chat.id, gemini_response, "🌐 Translate Result")
            return

        except Exception as e:
            logging.exception(f"Error during translation: {e}")
            bot.send_message(message.chat.id, "❌ An unexpected error occurred during translation. Please try again.")
            return
...
def call_gemini_api(prompt):
    try:
        ...
        if 'candidates' in gemini_response and 'parts' in gemini_response['candidates'][0]['content'] and gemini_response['candidates'][0]['content']['parts'][0]['text']:
            return gemini_response['candidates'][0]['content']['parts'][0]['text']
        else:
            return "❌ Gemini API failed to provide a response."
    except requests.exceptions.RequestException as e:
        logging.error(f"Gemini API network error: {e}")
        return "❌ A network error occurred while contacting the Gemini API."
    except Exception as e:
        logging.exception(f"Unhandled error in Gemini API call: {e}")
        return "❌ An unexpected error occurred while processing the request with the Gemini API."

# Helper function to get transcription content from string or file
def get_transcription_content(original_message_id):
    content = last_transcription.get(str(original_message_id))
    if content is None:
        return None
    ...
    if isinstance(content, str) and os.path.exists(content):
        try:
            ...
            logging.error(f"Error reading transcription file {content}: {e}")
            return None
    
    # Otherwise, assume it's a string
    return content

# Helper function to send Gemini API response to the user
def send_gemini_response_to_user(chat_id, response_text, title):
    if len(response_text) > 4000:
        # Create a temporary file and send it
        temp_file_path = f"{title.split()[1].lower()}_result_{uuid.uuid4()}.txt"
        with open(temp_file_path, 'w', encoding='utf-8') as f:
            f.write(response_text)
        
        with open(temp_file_path, 'rb') as f:
            bot.send_document(chat_id, f, caption=f"**{title}**\n\nThe result is too long. Here is the file:", parse_mode="Markdown")
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("summarize|"))
def summarize_callback(call):
    ...
    if transcription_content is None:
        bot.answer_callback_query(call.id, "❌ Sorry, the transcription text could not be found.")
        return
    ...
    # Use the new helper function to send the response
    send_gemini_response_to_user(chat_id, gemini_response, "📝 Summarize Result")

    # Clean up the temporary file if it was created for transcription
    content = last_transcription.get(str(original_message_id))
    if isinstance(content, str) and os.path.exists(content):
        os.remove(content)
        del last_transcription[str(original_message_id)]
...
def translate_callback(call):
    ...
    user_translate_mode[user_id] = original_message_id_str
    
    bot.send_message(
        chat_id,
        "Please enter the language you would like to translate the text to (e.g., Somali, English, Spanish)."
    )
    bot.answer_callback_query(call.id)
