...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        welcome_message = (
            ...
            "   - Send a YouTube link, and I'll send you a summary!\n\n"
...
def help_handler(message):
    ...
    help_text = (
        "📖 **How to use This Bot?**\n\n"
        "This bot makes it easy to convert text to audio, audio/video to text, or summarize YouTube videos. Here's how it works:\n\n"
        "⸻\n"
        "**1. Convert Text to Audio (TTS):**\n"
        ...
        "⸻\n"
        "**3. Summarize YouTube Videos:**\n"
        "• Send me a valid YouTube video link, and I will provide a summary for it. I can't summarize videos longer than 2 hours.\n\n"
        "⸻\n"
        "**4. Your Data & Privacy:**\n"
...
# New function to handle YouTube video summaries
async def summarize_youtube_video(chat_id, user_id_for_settings, video_url):
    ...
    try:
        processing_msg = bot.send_message(chat_id, "🔍 Searching for the video...")
        ...
        with yt_dlp.YoutubeDL(ydl_opts_info) as ydl:
            ...
            if duration and duration > 7200:  # 2 hours in seconds
                bot.send_message(chat_id, "❌ I cannot summarize videos longer than 2 hours.")
                return

        bot.edit_message_text("⬇️ Downloading audio...", chat_id, processing_msg.message_id)
        ...
        bot.edit_message_text("✍️ Transcribing the audio...", chat_id, processing_msg.message_id)
        ...
        transcript_res = requests.post("https://api.assemblyai.com/v2/transcript",
            headers={"authorization": ASSEMBLYAI_API_KEY, "content-type": "application/json"},
            json={"audio_url": audio_url, "language_code": "en", "speech_model": "best"})
        transcript_res.raise_for_status()
        transcript_id = transcript_res.json().get("id")
        ...
        if res['status'] != 'completed':
            raise Exception(f"AssemblyAI transcription failed: {res.get('error', 'Unknown error')}")

        transcript_text = res.get("text", "")
        if not transcript_text:
            bot.send_message(chat_id, "ℹ️ No text could be extracted from the video's audio.")
            return

        bot.edit_message_text("🧠 Summarizing the content...", chat_id, processing_msg.message_id)

        # Summarize the transcript using Gemini API
        prompt_text = f"Summarize this video content clearly and effectively in a way that is engaging. Do not use the words 'this text' but instead use 'this video':\n\n{transcript_text}"
        ...
        bot.send_message(chat_id, f"📝 **Video Summary:**\n\n{summary}", parse_mode="Markdown")

    except Exception as e:
        logging.exception(f"Error summarizing YouTube video: {e}")
        bot.send_message(chat_id, f"❌ An error occurred while processing the video. Please check the link and try again.")
    finally:
        if processing_msg:
            try:
                bot.delete_message(chat_id, processing_msg.message_id)
...
def handle_text_for_tts_or_mode_input_common(message, user_id_for_settings: str):
    ...
    # Check for YouTube, TikTok, Facebook, Instagram, X, and Snapchat links
    media_link_regex = r"(https?://)?(www\.)?(youtube\.com/watch\?v=|youtu\.be/|youtube\.com/shorts/|tiktok\.com|facebook\.com|instagram\.com|x\.com|snapchat\.com)([^&=%\?\s]{11}|[^\s]+)"
    match = re.search(media_link_regex, message.text)
    if match:
        video_url = match.group(0)
        threading.Thread(
            target=lambda: asyncio.run(summarize_youtube_video(message.chat.id, user_id_for_settings, video_url))
        ).start()
        return
