    ...
    try:
        ...
        if res['status'] == 'completed':
            ...
            if not text:
                ...
                markup = InlineKeyboardMarkup()
                # (Isbeddel cusub halkan) - Waxaan beddelay qoraalka batoonka
                markup.add(InlineKeyboardButton("Summarize", callback_data=f"summarize|{original_message_id}"))
...
# (Isbeddel cusub halkan) - Waxaan beddelay magaca callback-ga iyo prompt-ka.
...
def summarize_callback(call):
    ...
    # Simple check for language name, defaulting to English
    lang_name = next((name for name, code in STT_LANGUAGES.items() if code == lang_code), "English")

    # (Isbeddel cusub halkan) - Waxaan ka saaray 'and review' prompt-ka.
    prompt = f"Fadlan soo koob qoraalkan soo socda ee luqadda asalka ah ({lang_name}):\n\n{transcription_text}"

    bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
        text="Waxaan ka shaqaynayaa Gemini API... Wax yar sug.",
        reply_markup=None
    )
