...
user_custom_summary_mode = {} # New state variable for custom summary mode
...
def start_handler(message):
    ...
    user_custom_summary_mode[user_id_str] = None
...
def help_handler(message):
    ...
    user_custom_summary_mode[user_id] = None
...
def handle_language_stt_command(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    user_pitch_input_mode[user_id_for_settings] = None
    user_rate_input_mode[user_id_for_settings] = None
    user_custom_summary_mode[user_id_for_settings] = None
...
def handle_stt_language_select_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("Get Summarize & Review", callback_data="summarize_review"),
        InlineKeyboardButton("Custom", callback_data="custom_summary")
    )
...
@bot.callback_query_handler(lambda c: c.data == "summarize_review")
def on_summarize_review_button(call):
    uid = str(call.from_user.id)
    user_custom_summary_mode[uid] = "default"
    bot.answer_callback_query(call.id, "Default summary and review mode selected.")
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text="Default summary and review mode selected. Now, please send the transcription text.",
    )

@bot.callback_query_handler(lambda c: c.data == "custom_summary")
def on_custom_summary_button(call):
    uid = str(call.from_user.id)
    user_custom_summary_mode[uid] = "awaiting_prompt"
    bot.answer_callback_query(call.id, "Custom summary mode selected.")
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text="Custom summary mode selected. Please send your custom prompt text now (e.g., 'Summarize this in five bullet points and check for grammar errors')."
    )

def call_gemini_api(text: str, user_id: str, prompt: str = None) -> str:
    GEMINI_API_URL = (
    ...
    if prompt:
        ...
        system_instruction = (
            "You are an AI assistant. You will be given a transcription text. "
            "Your task is to: "
            "1. **Summarize** the key points of the text concisely. "
            "2. **Review** the accuracy and quality of the transcription. Point out any potential errors or unclear parts. "
            "Please provide the response in the original language of the transcription. "
            "Format the response clearly with headings for 'Summary' and 'Review'."
        )
    ...
    try:
        ...
        if 'candidates' in response_data and response_data['candidates']:
            ...
            if 'content' in candidate and 'parts' in candidate['content']:
                ...
                logging.error(f"Gemini API response has no content parts: {response_data}")
                return "Error: Gemini API response format is unexpected."
        else:
            logging.error(f"Gemini API response has no candidates: {response_data}")
    ...
    try:
        ...
        if res['status'] == 'completed':
            ...
            if not text:
                ...
                markup = InlineKeyboardMarkup(row_width=1)
                markup.add(
                    InlineKeyboardButton("Get Summarize & Review", callback_data=f"gemini_request|{original_message_id}|{text[:200]}"),
                    InlineKeyboardButton("Custom", callback_data="custom_summary")
                )
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    # Handle custom summary prompt
    if user_custom_summary_mode.get(user_id_for_settings) == "awaiting_prompt":
        prompt = message.text
        user_custom_summary_mode[user_id_for_settings] = "awaiting_transcription"
        user_custom_summary_mode[f"{user_id_for_settings}_prompt"] = prompt
        target_bot.send_message(message.chat.id, "I've saved your custom prompt. Now, please send the transcription text you want to process.")
        return

    if user_custom_summary_mode.get(user_id_for_settings) == "awaiting_transcription":
        prompt = user_custom_summary_mode.pop(f"{user_id_for_settings}_prompt", None)
        text_to_process = message.text
        target_bot.send_message(message.chat.id, "Processing your request with the custom prompt...")
        response = call_gemini_api(text_to_process, user_id_for_settings, prompt=prompt)
        target_bot.send_message(message.chat.id, response)
        user_custom_summary_mode[user_id_for_settings] = None
        return
...
def handle_unsupported_media_types(message):
    ...
    user_custom_summary_mode[uid] = None
