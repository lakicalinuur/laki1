...
GEMINI_API_KEY = "AlzaSyCHrGhRKXAp3DuQGH8HLB60ggryZeU-FA9E"
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        welcome_message = (
            ...
            "   - Send a YouTube link and I will summarize it for you.\n\n"
...
def help_handler(message):
    ...
    help_text = (
        ...
        "⸻\n"
        "**3. Summarize YouTube Videos:**\n"
        "• **Send a Link:** Simply send a YouTube video link to the bot.\n"
        "• **Get Summary:** The bot will automatically download the audio, transcribe it, and provide a concise summary.\n\n"
        "⸻\n"
        "**4. Your Data & Privacy:**\n"
...
async def download_youtube_audio(url: str, file_path: str):
    try:
        yt = YouTube(url)
...
async def summarize_youtube_video(chat_id: int, user_id: str, youtube_url: str, original_message_id: int):
    processing_msg = None
    audio_file_path = None
    try:
        processing_msg = bot.send_message(chat_id, "⏳ Downloading audio from the video...", reply_to_message_id=original_message_id)

        yt = YouTube(youtube_url)
        video_length_seconds = yt.length

        if video_length_seconds > 2 * 3600:  # 2 hours in seconds
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=processing_msg.message_id,
                text="❌ This video is longer than 2 hours. I can't summarize videos that long."
            )
            return

        audio_file_path = f"yt_audio_{uuid.uuid4()}.mp4"
        success = await download_youtube_audio(youtube_url, audio_file_path)

        if not success:
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=processing_msg.message_id,
                text="❌ Failed to download audio. The video might be restricted or unavailable."
            )
            return
        ...
        if res['status'] != 'completed' or not res.get("text"):
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=processing_msg.message_id,
                text="❌ Transcription failed or no text was found in the video."
            )
            return
        ...
        bot.edit_message_text(
            chat_id=chat_id,
            message_id=processing_msg.message_id,
            text="📝 Summarizing the transcript..."
        )

        prompt = f"""
        Summarize the following video transcript in a clean, attractive, and engaging way.
        Do not use the words "this text" or "this transcript". Instead, refer to it as "this video".
        The summary should be concise and easy to read, highlighting the key points.
        ...
        summary = gemini_res.json().get("candidates")[0].get("content").get("parts")[0].get("text")
        
        if summary:
            final_message = f"**🎥 Video Summary: {yt.title}**\n\n{summary}"
            bot.send_message(chat_id, final_message, parse_mode="Markdown")
        else:
            bot.send_message(chat_id, "❌ I failed to create a summary. The video might be too complex or too short.")
        
        increment_processing_count_in_memory(user_id, "youtube_summary")

    except requests.exceptions.RequestException as e:
        logging.error(f"API error during YouTube summary processing: {e}")
        bot.send_message(chat_id, f"❌ An API error occurred: {e}")
    except Exception as e:
        logging.exception(f"Unhandled error during YouTube summary processing: {e}")
        ...
        if audio_file_path and os.path.exists(audio_file_path):
            try:
                ...
                logging.error(f"Error deleting audio file {audio_file_path}: {e}")
...
def handle_text_for_tts_or_mode_input_common(message, user_id_for_settings: str):
    ...
    if is_youtube_link(message.text):
        threading.Thread(
            target=lambda: asyncio.run(summarize_youtube_video(message.chat.id, user_id_for_settings, message.text, message.message_id))
        ).start()
        return
...
def set_bot_info_and_startup():
    ...
    if check_db_connection():
        ...
        try:
            ...
            logging.error(f"Failed to set webhook without DB connection: {e}")
        set_bot_commands()
