...
def get_child_bot_info_in_memory(token: str) -> dict | None:
    ...
    if db_bot:
        in_memory_data["registered_bots"][token] = db_bot
    return db_bot
...
def on_register_bot_service_select(call):
    ...
    if not bot_token:
        ...
        bot.send_message(chat_id=call.message.chat.id, text="Something went wrong. Use `/reg` again.")
        user_register_bot_mode[uid] = None
        return
    ...
    try:
        ...
        # Delete the old message with the buttons to avoid confusion
        bot.delete_message(call.message.chat.id, call.message.message_id)

        logging.info(f"Webhook set for child bot {bot_username} to {child_bot_webhook_url}")
    except telebot.apihelper.ApiTelegramException as e:
...
def set_webhook_on_startup():
    try:
        ...
        all_registered_bots = registered_bots_collection.find()
        for info in all_registered_bots:
            token = info["_id"]
...
def set_bot_info_and_startup():
    ...
    if check_db_connection():
        ...
        # If DB fails, rely on in-memory, but warn the admin.
        # This is a fallback and not a recommended state.
        set_webhook_on_startup()
        set_bot_commands()
