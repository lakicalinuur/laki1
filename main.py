    ...
    try:
        ...
        if res['status'] == 'completed':
            ...
            if not text:
                ...
                markup = InlineKeyboardMarkup()
                markup.add(InlineKeyboardButton("Get Summarize & Review", callback_data=f"summarize_review|{original_message_id}"))
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("summarize_review|"))
def summarize_review_callback(call):
    chat_id = call.message.chat.id
    ...
    prompt = f"Please summarize and review the following transcription text in the language of the original text ({lang_name}):\n\n{transcription_text}"

    bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
        text="Processing with Gemini API... This might take a moment.",
        reply_markup=None
    )
    ...
    bot.send_message(
        chat_id=chat_id,
        text=f"**📝 Summarize & Review Result**\n\n{gemini_response}",
        parse_mode="Markdown"
    )
    bot.answer_callback_query(call.id)
