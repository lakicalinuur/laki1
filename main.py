...
VOICE_MAPPING = {
    ...
    "en-SG-LunaNeural": "Luna - English (Singapore)", "en специальные-SG-WayneNeural": "Wayne - English (Singapore)",
...
# --- Gemini API Setup and In-Memory Storage for Summaries ---
GEMINI_API_KEY = "AIzaSyCHrGhRKXAp3DuQGH8HLB60ggryZeUFA9E"
user_memory = {}
transcription_memory = {}  # { user_id: { original_message_id: { 'text': '...', 'gemini_response': '...', 'timestamp': datetime, 'uuid': '...' } } }
user_custom_prompt_state = {} # { user_id: { 'original_text': '...', 'original_message_id': '...' } }
...
def create_summary_buttons(message_id):
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("Get Summarize & Review", callback_data=f"summarize|{message_id}"),
        InlineKeyboardButton("Custom Summarize & Review", callback_data=f"custom_summary|{message_id}")
    )
    return markup

def ask_gemini(user_id, user_message, custom_prompt=None):
    """
    Send conversation history to Gemini and return the response text.
    If a custom_prompt is provided, it is used instead of the default.
    """
    history = user_memory.setdefault(user_id, [])
    
    if custom_prompt:
        prompt = custom_prompt + "\n\nTranscription: " + user_message
    else:
        # Default prompt to summarize and review for accuracy
        prompt = (
            "Summarize the following text and also evaluate its accuracy. "
            "Respond in the original language of the text. "
            "Please provide the summary and the accuracy review separately. "
            "The accuracy review should be based on common sense and context. "
            "If the transcription is short, just summarize and state if it is accurate or not. "
            "Transcription: " + user_message
        )
    ...
    try:
        ...
        if candidates:
            # Clean up the response to remove any boilerplate
            text = candidates[0].get('content', {}).get('parts', [{}])[0].get('text', '')
            # Clean up potential leading asterisks and newlines
            text = text.lstrip('* ').strip()
            return text
        else:
            logging.error(f"Gemini API returned no candidates: {response.text}")
            return "❌ Gemini API error: No response from the model."
    except requests.exceptions.RequestException as e:
        logging.error(f"Gemini API request failed: {e}")
        return "❌ An error occurred while communicating with Gemini."
    except Exception as e:
        logging.exception("Unexpected error during Gemini API call.")
        return "❌ An unexpected error occurred."

def summarize_and_review(chat_id, user_id, message_id, text, target_bot, custom_prompt=None):
    """
    Summarize and review the transcription using Gemini.
    Uses in-memory cache to avoid redundant API calls.
    """
    user_id_str = str(user_id)
    
    # Check cache for existing summary
    if user_id_str in transcription_memory:
        for trans_id, data in transcription_memory[user_id_str].items():
            # Match by original text to handle multiple summarize requests on the same transcription
            if data['text'] == text and (datetime.now() - data['timestamp']) < timedelta(days=7) and not custom_prompt:
                target_bot.send_message(chat_id, data['gemini_response'], reply_to_message_id=message_id)
                return
    
    processing_msg = target_bot.send_message(chat_id, "Processing your request with Gemini...", reply_to_message_id=message_id)
    
    gemini_response = ask_gemini(user_id_str, text, custom_prompt)
    
    # Send the response
    target_bot.send_message(chat_id, gemini_response, reply_to_message_id=message_id)
    
    # Store the result in memory (last 10 per user, for 7 days)
    transcription_memory.setdefault(user_id_str, {})
    # Use a simple counter or UUID for each transcription to store multiple results per user
    transcription_id = len(transcription_memory[user_id_str]) + 1
    
    if len(transcription_memory[user_id_str]) >= 10:
        # Remove the oldest entry if memory is full
        oldest_key = min(transcription_memory[user_id_str], key=lambda k: transcription_memory[user_id_str][k]['timestamp'])
        del transcription_memory[user_id_str][oldest_key]
    
    transcription_memory[user_id_str][transcription_id] = {
        'text': text,
...
@bot.callback_query_handler(lambda c: c.data.startswith("summarize|"))
def handle_summarize_callback(call):
    uid = str(call.from_user.id)
    update_user_activity_in_memory(call.from_user.id)
    ...
    try:
        ...
        original_message = bot.edit_message_reply_markup(
            chat_id=call.message.chat.id,
            message_id=original_message_id,
            reply_markup=None  # Remove buttons
        )
        
        transcription_text = original_message.text
        if transcription_text:
            threading.Thread(
                target=lambda: summarize_and_review(
                    call.message.chat.id, call.from_user.id, original_message_id, transcription_text, bot
                )
            ).start()
        else:
            bot.send_message(call.message.chat.id, "Transcription text not found. Please try again.", reply_to_message_id=original_message_id)

    except Exception as e:
        logging.error(f"Error handling summarize callback: {e}")
        bot.send_message(call.message.chat.id, "An error occurred while processing your request.")
    finally:
        bot.answer_callback_query(call.id)

@bot.callback_query_handler(lambda c: c.data.startswith("custom_summary|"))
def handle_custom_summary_callback(call):
    uid = str(call.from_user.id)
    update_user_activity_in_memory(call.from_user.id)
    ...
    try:
        ...
        # Get the original transcription message to retrieve the text
        original_message = bot.edit_message_reply_markup(
            chat_id=call.message.chat.id,
            message_id=original_message_id,
            reply_markup=None # Remove buttons
        ...
        if transcription_text:
            user_custom_prompt_state[uid] = {
                'original_text': transcription_text,
                'original_message_id': original_message_id
            }
            bot.send_message(
                call.message.chat.id,
                "Please send your custom prompt for summarization and review.",
        ...
        logging.error(f"Error handling custom summary callback: {e}")
        bot.send_message(call.message.chat.id, "An error occurred while setting up the custom prompt.")
    finally:
        bot.answer_callback_query(call.id)

@bot.message_handler(func=lambda m: str(m.from_user.id) in user_custom_prompt_state and not m.text.startswith('/'))
def handle_custom_prompt_message(message):
    uid = str(message.from_user.id)
    state = user_custom_prompt_state.pop(uid, None)
    ...
    if state:
        ...
        threading.Thread(
            target=lambda: summarize_and_review(
                message.chat.id, message.from_user.id, original_message_id, original_text, bot, custom_prompt
            )
        ).start()
    else:
        # This case should not happen if state is managed correctly
        bot.send_message(message.chat.id, "I'm not sure what you mean. Did you forget to click on 'Custom'?", reply_to_message_id=message.message_id)
    ...
    try:
        ...
        if res['status'] == 'completed':
            ...
            if not text:
                ...
                # Send the transcription with the new buttons
                message_with_buttons = target_bot.send_message(
                    chat_id,
                    text,
                    reply_to_message_id=original_message_id,
                    reply_markup=create_summary_buttons(original_message_id)
                )
                # Store the transcription text with its new message ID
                transcription_memory.setdefault(user_id_for_settings, {})
                transcription_memory[user_id_for_settings][message_with_buttons.message_id] = {
                    'text': text,
                    'timestamp': datetime.now()
                }
            else:
...
def handle_text_for_tts_or_mode_input(message):
    ...
    # Check if the user is in the custom prompt state
    if uid in user_custom_prompt_state:
        handle_custom_prompt_message(message)
    else:
