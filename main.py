...
# --- MongoDB Setup ---
MONGO_URI = "mongodb+srv://hoskasii:GHyCdwpI0PvNuLTg@cluster0.dy7oe7h.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
...
STT_LANGUAGES = {
    "English 🇬🇧": "en", "العربية 🇸🇦": "ar", "Spanish 🇪🇸": "es", "French 🇫🇷": "fr", "German 🇩🇪": "de", "Russian 🇷🇺": "ru",
    "Portuguese 🇵🇹": "pt", "Japanese 🇯🇵": "ja", "Korean 🇰🇷": "ko", "Chinese 🇨🇳": "zh", "Hindi 🇮🇳": "hi",
    "Indonesian 🇮🇩": "id", "Italian 🇮🇹": "it", "Turkish 🇹🇷": "tr", "Vietnamese 🇻🇳": "vi", "Thai 🇹🇱": "th",
    "Polish 🇵🇱": "pl", "Dutch 🇳🇱": "nl", "Swedish 🇸🇪": "sv", "Norwegian 🇳🇴": "no", "Danish 🇩🇰": "da",
    "Finnish 🇫🇮": "fi", "Czech 🇨🇿": "cs", "Hungarian 🇭🇺": "hu", "Romanian 🇷🇴": "ro", "Malay 🇲🇾": "ms",
    "Uzbek 🇺🇿": "uz", "Tagalog 🇵🇱": "tl", "اردو 🇵🇰": "ur", "Swahili 🇰🇪": "sw", "Kazakh 🇰🇿": "kk",
    "Azerbaijani 🇦🇿": "az", "Bulgarian 🇧🇬": "bg", "Serbian 🇷🇸": "sr", "فارسى 🇮🇷": "fa",
}
...
def get_user_data_in_memory(user_id: str) -> dict | None:
    ...
    if db_user: in_memory_data["users"][user_id] = db_user
    return db_user
...
def get_tts_user_voice_in_memory(user_id: str) -> str:
    ...
    if setting: return setting.get("voice", "en-US-EricNeural")
...
def set_tts_user_voice_in_memory(user_id: str, voice: str):
    if user_id not in in_memory_data["tts_settings"]: in_memory_data["tts_settings"][user_id] = {}
...
def get_tts_user_pitch_in_memory(user_id: str) -> int:
    setting = in_memory_data["tts_settings"].get(user_id)
    if setting: return setting.get("pitch", 0)
    db_setting = tts_settings_collection.find_one({"_id": user_id})
    if db_setting:
        in_memory_data["tts_settings"][user_id] = db_setting
...
def get_tts_user_rate_in_memory(user_id: str) -> int:
    setting = in_memory_data["tts_settings"].get(user_id)
    if setting: return setting.get("rate", 0)
    db_setting = tts_settings_collection.find_one({"_id": user_id})
    if db_setting:
        in_memory_data["tts_settings"][user_id] = db_setting
...
def get_stt_user_lang_in_memory(user_id: str) -> str:
    ...
    if setting: return setting.get("language_code", "en")
...
def set_stt_user_lang_in_memory(user_id: str, lang_code: str):
    if user_id not in in_memory_data["stt_settings"]: in_memory_data["stt_settings"][user_id] = {}
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        welcome_message = (
            ...
            "   - Just send me a YouTube link. I'll get the summary for you!\n"
            "   - Maximum video length is **2 hours**.\n\n"
...
def help_handler(message):
    ...
    help_text = (
        ...
        "⸻\n"
        "**3. Summarize YouTube Videos:**\n"
        "• **Send the link:** Just send me a YouTube video link to get a quick summary. "
        "• **Note:** The maximum video length is **2 hours**.\n\n"
        "⸻\n"
        "**4. Your Data & Privacy:**\n"
...
TTS_VOICES_BY_LANGUAGE = {
    ...
    "Hindi": ["hi-IN-SwaraNeural", "hi-IN-MadhurNeural"], "French": ["fr-FR-DeniseNeural", "fr-FR-HenriNeural", "fr-CA-SylvieNeural", "fr-CA-JeanNeural", "fr-CH-ArianeNeural", "fr-CH-FabriceNeural", "fr-CH-GerardNeural"],
    ...
    "Japanese": ["ja-JP-NanamiNeural", "ja-JP-KeitaNeural"], "Portuguese": ["pt-BR-FranciscaNeural", "pt-BR-AntonioNeural", "pt-PT-RaquelNeural", "pt-PT-DuarteNeural"],
    "Russian": ["ru-RU-SvetlanaNeural", "ru-RU-DmitryNeural", "ru-RU-LarisaNeural", "ru-RU-MaximNeural"],
    "Turkish": ["tr-TR-EmelNeural", "tr-TR-AhmetNeural"], "Korean": ["ko-KR-SunHiNeural", "ko-KR-InJoonNeural"],
    "Italian": ["it-IT-ElsaNeural", "it-IT-DiegoNeural"], "Indonesian": ["id-ID-GadisNeural", "id-ID-ArdiNeural"],
    "Vietnamese": ["vi-VN-HoaiMyNeural", "vi-VN-NamMinhNeural"], "Thai": ["th-TH-PremwadeeNeural", "th-TH-NiwatNeural"],
    "Dutch": ["nl-NL-ColetteNeural", "nl-NL-MaartenNeural"], "Polish": ["pl-PL-ZofiaNeural", "pl-PL-MarekNeural"],
    "Swedish": ["sv-SE-SofieNeural", "sv-SE-MattiasNeural"], "Filipino": ["fil-PH-BlessicaNeural", "fil-PH-AngeloNeural"],
    "Greek": ["el-GR-AthinaNeural", "el-GR-NestorasNeural"], "Hebrew": ["he-IL-AvriNeural", "he-IL-HilaNeural"],
    "Hungarian": ["hu-HU-NoemiNeural", "hu-HU-AndrasNeural"], "Czech": ["cs-CZ-VlastaNeural", "cs-CZ-AntoninNeural"],
    "Danish": ["da-DK-ChristelNeural", "da-DK-JeppeNeural"], "Finnish": ["fi-FI-SelmaNeural", "fi-FI-HarriNeural"],
    "Norwegian": ["nb-NO-PernilleNeural", "nb-NO-FinnNeural"], "Romanian": ["ro-RO-AlinaNeural", "ro-RO-EmilNeural"],
    "Slovak": ["sk-SK-LukasNeural", "sk-SK-ViktoriaNeural"], "Ukrainian": ["uk-UA-PolinaNeural", "uk-UA-OstapNeural"],
    "Malay": ["ms-MY-YasminNeural", "ms-MY-OsmanNeural"], "Bengali": ["bn-BD-NabanitaNeural", "bn-BD-BasharNeural"],
    "Urdu": ["ur-PK-AsmaNeural", "ur-PK-FaizanNeural"], "Nepali": ["ne-NP-SagarNeural", "ne-NP-HemkalaNeural"],
    "Sinhala": ["si-LK-SameeraNeural", "si-LK-ThiliniNeural"], "Lao": ["lo-LA-ChanthavongNeural", "lo-LA-KeomanyNeural"],
    "Myanmar": ["my-MM-NilarNeural", "my-MM-ThihaNeural"], "Georgian": ["ka-GE-EkaNeural", "ka-GE-GiorgiNeural"],
    "Armenian": ["hy-AM-AnahitNeural", "hy-AM-AraratNeural"], "Azerbaijani": ["az-AZ-BabekNeural", "az-AZ-BanuNeural"],
    "Uzbek": ["uz-UZ-MadinaNeural", "uz-UZ-SuhrobNeural"], "Serbian": ["sr-RS-NikolaNeural", "sr-RS-SophieNeural"],
    "Croatian": ["hr-HR-GabrijelaNeural", "hr-HR-SreckoNeural"], "Slovenian": ["sl-SI-PetraNeural", "sl-SI-RokNeural"],
    "Latvian": ["lv-LV-EveritaNeural", "lv-LV-AnsisNeural"], "Lithuanian": ["lt-LT-OnaNeural", "lt-LT-LeonasNeural"],
    "Amharic": ["am-ET-MekdesNeural", "am-ET-AbebeNeural"], "Swahili": ["sw-KE-ZuriNeural", "sw-KE-RafikiNeural"],
    "Zulu": ["zu-ZA-ThandoNeural", "zu-ZA-ThembaNeural"], "Afrikaans": ["af-ZA-AdriNeural", "af-ZA-WillemNeural"],
    "Somali": ["so-SO-UbaxNeural", "so-SO-MuuseNeural"], "Persian": ["fa-IR-DilaraNeural", "fa-IR-ImanNeural"],
    "Mongolian": ["mn-MN-BataaNeural", "mn-MN-YesuiNeural"], "Maltese": ["mt-MT-GraceNeural", "mt-MT-JosephNeural"],
    "Irish": ["ga-IE-ColmNeural", "ga-IE-OrlaNeural"], "Albanian": ["sq-AL-AnilaNeural", "sq-AL-IlirNeural"]
}
...
def make_tts_language_keyboard():
    ...
    for i in range(0, len(buttons), 3): markup.add(*buttons[i:i+3])
    return markup
...
def handle_rate_command(message, user_id_for_settings: str):
...
def handle_rate_callback(call, user_id_for_settings: str):
    chat_id = call.message.chat.id
...
def cmd_voice_rate(message):
    ...
    if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.chat.id):
        send_subscription_message(message.chat.id)
        return
    handle_rate_command(message, uid)
...
def on_rate_set_callback(call):
    ...
    if call.message.chat.type == 'private' and str(call.from_user.id) != str(ADMIN_ID) and not check_subscription(call.message.chat.id):
        ...
        bot.answer_callback_query(call.id)
        return
    handle_rate_callback(call, uid)

def handle_pitch_command(message, user_id_for_settings: str):
    chat_id = message.chat.id
    user_tts_mode[user_id_for_settings] = None
...
def handle_pitch_callback(call, user_id_for_settings: str):
    chat_id = call.message.chat.id
    user_pitch_input_mode[user_id_for_settings] = None
    try:
...
def cmd_voice_pitch(message):
    ...
    if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.chat.id):
        send_subscription_message(message.chat.id)
        return
    handle_pitch_command(message, uid)
...
def on_pitch_set_callback(call):
    ...
    if call.message.chat.type == 'private' and str(call.from_user.id) != str(ADMIN_ID) and not check_subscription(call.message.chat.id):
        ...
        bot.answer_callback_query(call.id)
        return
    handle_pitch_callback(call, uid)

def handle_voice_command(message, user_id_for_settings: str):
    chat_id = message.chat.id
    user_tts_mode[user_id_for_settings] = None
    user_pitch_input_mode[user_id_for_settings] = None
    user_rate_input_mode[user_id_for_settings] = None
...
def handle_tts_language_select_callback(call, user_id_for_settings: str):
    chat_id = call.message.chat.id
    user_pitch_input_mode[user_id_for_settings] = None
    user_rate_input_mode[user_id_for_settings] = None
...
def handle_tts_voice_change_callback(call, user_id_for_settings: str):
    chat_id = call.message.chat.id
    user_pitch_input_mode[user_id_for_settings] = None
    user_rate_input_mode[user_id_for_settings] = None
...
def handle_tts_back_to_languages_callback(call, user_id_for_settings: str):
    chat_id = call.message.chat.id
    user_tts_mode[user_id_for_settings] = None
    user_pitch_input_mode[user_id_for_settings] = None
    user_rate_input_mode[user_id_for_settings] = None
...
def cmd_text_to_speech(message):
    ...
    if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.chat.id):
        send_subscription_message(message.chat.id)
        return
    handle_voice_command(message, user_id)
...
def on_tts_language_select(call):
    ...
    if call.message.chat.type == 'private' and str(call.from_user.id) != str(ADMIN_ID) and not check_subscription(call.message.chat.id):
        ...
        bot.answer_callback_query(call.id)
        return
    handle_tts_language_select_callback(call, uid)
...
def on_tts_voice_change(call):
    ...
    if call.message.chat.type == 'private' and str(call.from_user.id) != str(ADMIN_ID) and not check_subscription(call.message.chat.id):
        ...
        bot.answer_callback_query(call.id)
        return
    handle_tts_voice_change_callback(call, uid)
...
def on_tts_back_to_languages(call):
    ...
    if call.message.chat.type == 'private' and str(call.from_user.id) != str(ADMIN_ID) and not check_subscription(call.message.chat.id):
        ...
        bot.answer_callback_query(call.id)
        return
    handle_tts_back_to_languages_callback(call, uid)

async def synth_and_send_tts(chat_id: int, user_id_for_settings: str, text: str):
    text = text.replace('.', ',')
    ...
    try:
        ...
        if not os.path.exists(filename) or os.path.getsize(filename) == 0:
            bot.send_message(chat_id, "❌ I failed to create the audio. Try another text.")
            return
        with open(filename, "rb") as f:
            ...
            bot.send_audio(chat_id, f, caption=caption_text, parse_mode="Markdown")
        increment_processing_count_in_memory(user_id_for_settings, "tts")
    except MSSpeechError as e:
        logging.error(f"TTS error: {e}")
        bot.send_message(chat_id, "❌ An error occurred while generating the audio. Try again or choose another voice.")
    except Exception as e:
        logging.exception("TTS error")
        bot.send_message(chat_id, "This voice is not available, please choose another one.")
    finally:
        stop_recording.set()
...
def handle_language_stt_command(message, user_id_for_settings: str):
    chat_id = message.chat.id
    user_tts_mode[user_id_for_settings] = None
    user_pitch_input_mode[user_id_for_settings] = None
    user_rate_input_mode[user_id_for_settings] = None
    bot.send_message(chat_id, "Choose the language of your voice, audio, or video file for transcription using the buttons below:", reply_markup=build_stt_language_keyboard(), parse_mode="Markdown")

def handle_stt_language_select_callback(call, user_id_for_settings: str):
    chat_id = call.message.chat.id
    ...
    bot.answer_callback_query(call.id, f"✅ The language is {lang_name}!")
    bot.edit_message_text(
        chat_id=chat_id, message_id=call.message.message_id,
...
def send_stt_language_prompt(message):
    ...
    if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.from_user.id):
        send_subscription_message(message.chat.id)
        return
    handle_language_stt_command(message, user_id)
...
def on_stt_language_select(call):
    ...
    if call.message.chat.type == 'private' and str(call.from_user.id) != str(ADMIN_ID) and not check_subscription(call.message.chat.id):
        ...
        bot.answer_callback_query(call.id)
        return
    handle_stt_language_select_callback(call, uid)

async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, original_message_id: int):
    processing_msg = None
    downloaded_file_path = None
    try:
        processing_msg = bot.send_message(chat_id, " Processing...", reply_to_message_id=original_message_id)
        file_info = bot.get_file(file_id)
        if file_info.file_size > 20 * 1024 * 1024:
            bot.send_message(chat_id, "⚠️ The file is too large. Maximum size is 20MB. Send a smaller file.", reply_to_message_id=original_message_id)
            return
        downloaded_file_path = f"stt_temp_{uuid.uuid4()}_{file_info.file_path.split('/')[-1]}"
        downloaded_file = bot.download_file(file_info.file_path)
        with open(downloaded_file_path, 'wb') as f: f.write(downloaded_file)
        processing_start_time = datetime.now()
        ...
        if not audio_url: raise Exception("AssemblyAI upload failed: No upload_url received.")
        ...
        if not transcript_id: raise Exception("AssemblyAI transcription request failed: No transcript ID received.")
        polling_url = f"https://api.assemblyai.com/v2/transcript/{transcript_id}"
        while True:
            res = requests.get(polling_url, headers={"authorization": ASSEMBLYAI_API_KEY}).json()
            if res['status'] in ['completed', 'error']: break
            time.sleep(2)
        if res['status'] == 'completed':
            text = res.get("text", "")
            if not text:
                bot.send_message(chat_id, "ℹ️ No text extracted from this file.", reply_to_message_id=original_message_id)
            elif len(text) <= 4000:
                bot.send_message(chat_id, text, reply_to_message_id=original_message_id)
            else:
                import io
                f = io.BytesIO(text.encode("utf-8"))
                f.name = "transcript.txt"
                bot.send_document(chat_id, f, caption="Your transcript is too long. Here is the file:", reply_to_message_id=original_message_id)
            increment_processing_count_in_memory(user_id_for_settings, "stt")
            status = "success"
        else:
            error_msg = res.get("error", "Unknown transcription error.")
            bot.send_message(chat_id, f"❌ Transcription error: {error_msg}", parse_mode="Markdown", reply_to_message_id=original_message_id)
        ...
        bot.send_message(chat_id, "❌ A network error occurred. Try again.", reply_to_message_id=original_message_id)
    except Exception as e:
        logging.exception(f"Unhandled error during STT processing: {e}")
        bot.send_message(chat_id, "The file is too large, send one smaller than 20MB.", reply_to_message_id=original_message_id)
    finally:
        if processing_msg:
            try: bot.delete_message(chat_id, processing_msg.message_id)
            except Exception as e: logging.error(f"Could not delete processing message: {e}")
        if downloaded_file_path and os.path.exists(downloaded_file_path):
            try:
                os.remove(downloaded_file_path)
                logging.info(f"Deleted temporary STT file: {downloaded_file_path}")
            except Exception as e: logging.error(f"Error deleting STT file {downloaded_file_path}: {e}")

async def summarize_youtube_video(chat_id: int, url: str, original_message_id: int):
    processing_msg = None
    audio_file_path = None
    try:
        processing_msg = bot.send_message(chat_id, "📥 Downloading video audio...", reply_to_message_id=original_message_id)
        yt = YouTube(url)
        video_stream = yt.streams.filter(only_audio=True).first()
        if not video_stream:
            bot.send_message(chat_id, "❌ Could not find an audio stream for this video.")
            return
        video_length_minutes = yt.length / 60
        if video_length_minutes > 120:
            bot.send_message(chat_id, "❌ Sorry, I can only summarize videos shorter than 2 hours.", reply_to_message_id=original_message_id)
            return
        audio_file_path = f"yt_audio_{uuid.uuid4()}.mp4"
        video_stream.download(filename=audio_file_path)
        bot.edit_message_text("✍️ Transcribing audio...", chat_id, processing_msg.message_id)
        with open(audio_file_path, "rb") as f:
            upload_res = requests.post("https://api.assemblyai.com/v2/upload",
                headers={"authorization": ASSEMBLYAI_API_KEY, "Content-Type": "application/octet-stream"}, data=f)
        upload_res.raise_for_status()
        audio_url = upload_res.json().get('upload_url')
        if not audio_url: raise Exception("AssemblyAI upload failed: No upload_url received.")
        transcript_res = requests.post("https://api.assemblyai.com/v2/transcript",
            headers={"authorization": ASSEMBLYAI_API_KEY, "content-type": "application/json"},
            json={"audio_url": audio_url, "speech_model": "best"})
        transcript_res.raise_for_status()
        transcript_id = transcript_res.json().get("id")
        if not transcript_id: raise Exception("AssemblyAI transcription request failed: No transcript ID received.")
        polling_url = f"https://api.assemblyai.com/v2/transcript/{transcript_id}"
        ...
        if not transcript_text:
            bot.send_message(chat_id, "ℹ️ No text was extracted from the video's audio.")
            return
        bot.edit_message_text("🧠 Summarizing with Gemini...", chat_id, processing_msg.message_id)
        gemini_headers = {"Content-Type": "application/json"}
        gemini_payload = {
            "contents": [{
                "parts": [{
                    "text": f"Summarize the following video transcript in a clean and attractive way. "
                            f"Start with 'This video explains...' or 'In this video...' instead of 'This text...':\n\n{transcript_text}"
                }]
            }]
        }
        gemini_url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key={GEMINI_API_KEY}"
        gemini_res = requests.post(gemini_url, headers=gemini_headers, data=json.dumps(gemini_payload))
        gemini_res.raise_for_status()
        gemini_summary = gemini_res.json().get('candidates', [{}])[0].get('content', {}).get('parts', [{}])[0].get('text', '❌ Could not get a summary.')
        bot.send_message(chat_id, f"📝 **YouTube Video Summary**\n\n{gemini_summary}", parse_mode="Markdown")
        increment_processing_count_in_memory(original_message_id, "youtube_summary")
    except requests.exceptions.RequestException as e:
        logging.error(f"API Error during YouTube summarization: {e}")
        bot.send_message(chat_id, "❌ A network or API error occurred. Try again.")
    except exceptions.VideoUnavailable:
        bot.send_message(chat_id, "❌ This YouTube video is unavailable or private.")
    except exceptions.AgeRestrictedError:
        bot.send_message(chat_id, "❌ This video is age-restricted and cannot be processed.")
    except Exception as e:
        logging.exception("An error occurred during YouTube summarization.")
        ...
        if audio_file_path and os.path.exists(audio_file_path):
            ...
            except Exception as e: logging.error(f"Error deleting audio file {audio_file_path}: {e}")
...
def handle_stt_media_types(message):
    ...
    if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.chat.id):
        send_subscription_message(message.chat.id)
        return
    handle_stt_media_types_common(message, uid)

def handle_stt_media_types_common(message, user_id_for_settings: str):
    ...
    if message.voice:
        ...
        if message.document.mime_type and (message.document.mime_type.startswith('audio/') or message.document.mime_type.startswith('video/')):
            ...
            bot.send_message(message.chat.id, "I'm sorry, I can only transcribe audio or video. Send a valid file.")
            return
    if not file_id:
        bot.send_message(message.chat.id, "I don't support this file type. Send audio, file or video.")
        return
    if user_id_for_settings not in in_memory_data["stt_settings"]:
        bot.send_message(message.chat.id, "First choose the transcription language click or type /lang")
        return
    threading.Thread(
        target=lambda: asyncio.run(process_stt_media(message.chat.id, user_id_for_settings, message_type, file_id, message.message_id))
    ).start()
...
def handle_text_for_tts_or_mode_input(message):
    ...
    if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.chat.id):
        send_subscription_message(message.chat.id)
        return
    if is_youtube_url(message.text.strip()):
        threading.Thread(
            target=lambda: asyncio.run(summarize_youtube_video(message.chat.id, message.text.strip(), message.message_id))
        ).start()
    else:
        handle_text_for_tts_or_mode_input_common(message, uid)

def handle_text_for_tts_or_mode_input_common(message, user_id_for_settings: str):
    update_user_activity_in_memory(int(user_id_for_settings))
    ...
    if current_voice:
        threading.Thread(
            target=lambda: asyncio.run(synth_and_send_tts(message.chat.id, user_id_for_settings, message.text))
        ).start()
    else:
        bot.send_message(message.chat.id, "You haven't chosen a voice yet! Use `/voice` first, then send me text. 🗣️")
