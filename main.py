...
# Dictionary to store the last transcription result for each user, mapped by original_message_id
last_transcription = {}
        ...
        if res['status'] == 'completed':
            ...
            if not text:
                ...
                if len(text) <= 4000:
                    ...
                    # Save the text to a .txt file in memory and send it
                    file_object = io.BytesIO(text.encode("utf-8"))
                    file_object.name = f"transcript_{original_message_id}.txt"
                    target_bot.send_document(
                        chat_id,
                        file_object,
                        caption="The transcript is too long to send as a message. Here is the file:",
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if user_translate_mode.get(uid) is not None:
        try:
            ...
            if str(original_message_id) not in last_transcription:
                bot.send_message(message.chat.id, "❌ Sorry, the transcription text could not be found.")
                return

            transcription_text = last_transcription.get(str(original_message_id))
...
def call_gemini_api(prompt):
    try:
        ...
        if 'candidates' in gemini_response and gemini_response['candidates'][0]['content']['parts'][0]['text']:
...
def translate_callback(call):
    ...
    if original_message_id_str not in last_transcription:
        bot.answer_callback_query(call.id, "❌ Sorry, the transcription text could not be found.")
        return

    user_translate_mode[user_id] = int(original_message_id_str)
