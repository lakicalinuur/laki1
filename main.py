...
def init_in_memory_data():
    ...
    try:
        ...
        all_registered_bots = registered_bots_collection.find()
        for bot_info in all_registered_bots:
            in_memory_data["registered_bots"][bot_info["_id"]] = bot_info
...
def update_user_activity_in_memory(user_id: int):
    ...
    user_data = in_memory_data["users"].get(user_id_str, {
        ...
        "youtube_summary_count": 0, "registered_bots": []
    })
...
def register_child_bot_in_memory(token: str, owner_id: str, service_type: str):
    bot_info = {
        "_id": token, "owner_id": owner_id, "service_type": service_type, "registration_date": datetime.now()
    }
    in_memory_data["registered_bots"][token] = bot_info
    registered_bots_collection.insert_one(bot_info)
    users_collection.update_one(
        {"_id": owner_id}, {"$push": {"registered_bots": token}}, upsert=True
    )
    logging.info(f"Child bot {token[:5]}... registered for owner {owner_id} with service {service_type} in memory and DB.")
    return True

def get_child_bot_info_in_memory(token: str) -> dict | None:
    bot_info = in_memory_data["registered_bots"].get(token)
    if bot_info: return bot_info
    db_bot = registered_bots_collection.find_one({"_id": token})
    if db_bot: in_memory_data["registered_bots"][token] = db_bot
    return db_bot
...
def register_bot_command(message):
    ...
    bot.send_message(message.chat.id, "Okay! To create your bot, send me your **Bot API Token**.\n\n"
...
def process_bot_token(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, f"Great! I have verified the token for @**{bot_info.username}**.\n\n"
        ...
        bot.send_message(message.chat.id, "❌ I cannot verify that token. It might be incorrect or deleted. Please check with @BotFather and try again.")
...
def on_register_bot_service_select(call):
    ...
    try:
        ...
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="❌ An error occurred while setting up your bot. Try again.")
    except Exception as e:
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        if processing_msg:
            try: target_bot.delete_message(chat_id, processing_msg.message_id)
...
async def summarize_youtube_video(chat_id: int, url: str, target_bot: telebot.TeleBot, original_message_id: int):
    processing_msg = None
    audio_file_path = None
    try:
        processing_msg = target_bot.send_message(chat_id, "📥 Downloading video audio...", reply_to_message_id=original_message_id)
        yt = YouTube(url)
        video_stream = yt.streams.filter(only_audio=True).first()
        if not video_stream:
            target_bot.send_message(chat_id, "❌ Could not find an audio stream for this video.")
            return
        video_length_minutes = yt.length / 60
        if video_length_minutes > 120:
            target_bot.send_message(chat_id, "❌ Sorry, I can only summarize videos shorter than 2 hours.", reply_to_message_id=original_message_id)
            return
        audio_file_path = f"yt_audio_{uuid.uuid4()}.mp4"
        video_stream.download(filename=audio_file_path)
        target_bot.edit_message_text("✍️ Transcribing audio...", chat_id, processing_msg.message_id)
        ...
        if not transcript_text:
            target_bot.send_message(chat_id, "ℹ️ No text was extracted from the video's audio.")
            return
        target_bot.edit_message_text("🧠 Summarizing with Gemini...", chat_id, processing_msg.message_id)
        ...
        target_bot.send_message(chat_id, f"📝 **YouTube Video Summary**\n\n{gemini_summary}", parse_mode="Markdown")
        ...
        target_bot.send_message(chat_id, "❌ A network or API error occurred. Try again.")
    except exceptions.VideoUnavailable:
        target_bot.send_message(chat_id, "❌ This YouTube video is unavailable or private.")
    except exceptions.AgeRestrictedError:
        target_bot.send_message(chat_id, "❌ This video is age-restricted and cannot be processed.")
    except Exception as e:
        logging.exception("An error occurred during YouTube summarization.")
        target_bot.send_message(chat_id, f"❌ An unexpected error occurred: {e}")
    finally:
        if processing_msg:
            try: target_bot.delete_message(chat_id, processing_msg.message_id)
            except Exception as e: logging.error(f"Could not delete processing message: {e}")
...
def handle_text_for_tts_or_mode_input(message):
    ...
    if is_youtube_url(message.text.strip()):
        threading.Thread(
            target=lambda: asyncio.run(summarize_youtube_video(message.chat.id, message.text.strip(), bot, message.message_id))
        ).start()
    else:
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if current_voice:
        ...
        target_bot.send_message(message.chat.id, "You haven't chosen a voice yet! Use `/voice` first, then send me text. 🗣️")
...
def child_webhook(child_bot_token):
    if request.method == "POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            if not user_id_for_settings: return "", 200
            if message:
                chat_id = message.chat.id
...
def set_webhook_on_startup():
    try:
        ...
        for token, info in registered_bots_collection.find().items():
